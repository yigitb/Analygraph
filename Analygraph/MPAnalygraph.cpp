...
MPAnalygraph::MPAnalygraph(unsigned w, unsigned h) {
	img_ = std::make_unique<Image>(w, h, 24);
}

MPAnalygraph::~MPAnalygraph() {
	img_.reset();
}

int MPAnalygraph::Compute(const Image* l, const Image* r, IAnalygraph::Mode mode) {
...
std::unique_ptr<Image> MPAnalygraph::GetImage() {
