...
MPSimdAnalygraph::MPSimdAnalygraph(unsigned w, unsigned h) {
	img_ = std::make_unique<Image>(w, h, 24);
}

MPSimdAnalygraph::~MPSimdAnalygraph() {
	img_.reset();
}

int MPSimdAnalygraph::Compute(const Image* l, const Image* r, IAnalygraph::Mode mode) {
...
std::unique_ptr<Image> MPSimdAnalygraph::GetImage() {
